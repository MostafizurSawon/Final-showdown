...
const dashboard = () =>{
  fetch("https://gardenhub-django.onrender.com/user/list/")
    .then((res) => res.json())
...
const displayProfile = (categories) => {
  //   console.log(services);
  const user = localStorage.getItem("user");
  console.log("USER->", user);
  categories.forEach((everyUser) => {
    const parent = document.getElementById("dashboard-profile");
    const li = document.createElement("div");
    // li.classList.add("nav-item");
    // console.log("USER->", user, everyUser);
    if(everyUser.username == user){
      console.log("inn");
      li.innerHTML = ` 
      <p>Username: ${everyUser.username}</p>
      <p>Name: ${everyUser.first_name +" "+ everyUser.last_name}</p>
      <p>Email: ${everyUser.email}</p>
    `;
    }
